...
bool Configuration::updateConfig(String config) {
	...
	if (currentConfig.WiFiClient && currentConfig.useNTP) {
		configTime(
			currentConfig.gmtOffset_sec,
			currentConfig.daylightOffset_sec,
			currentConfig.ntpServer1.c_str(),
			currentConfig.ntpServer2.c_str(),
			currentConfig.ntpServer3.c_str());
	} 
	// Set both GMT offset and DST offset
	TimeInterface::setOffset(currentConfig.gmtOffset_sec, currentConfig.daylightOffset_sec);
	return true;
}
